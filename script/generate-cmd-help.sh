...
for file in docs/macarchy-*.adoc; do
    subcommand=$(basename "$file" | sed 's/^macarchy-//' | sed 's/\.adoc$//' | sed 's/-/_/g')
    sed -n -E '/tag::synopsis/, /end::synopsis/ p' "$file" | \
        sed '1d' | \
        sed '$d' | \
        sed '/^$/ d' | \
        sed "1   s/^$macarchy_prefix/$____usage_prefix/" | \
        sed "2,$ s/^$macarchy_prefix/$_______or_prefix/" | \
