...
- `../Sources`.
  The majority of Macarchy source code. Managed by SPM `../Package.swift`
- `../Sources/AppBundle/`.
  macarchy.app server. Technically, it's a SPM library that is exposed to `xcode-app-bundle-launcher`
- `../Sources/Cli/`.
...
- `../xcode-app-bundle-launcher/`.
  ...
  The Xcode project model is located in `../macarchy.xcodeproj/` and generated by `../project.yml` "skeleton"
...
`macarchy` CLI binary is client. `macarchy.app` is server. Client and server talk to each other via predefined UNIX file.
...
Command checklist:
- [ ] Documentation in `../docs/macarchy-*` and `../docs/commands.adoc`
