...
export XCODEGEN_MACARCHY_CODE_SIGN_IDENTITY="macarchy-codesign-certificate"
...
while test $# -gt 0; do
    case $1 in
        ...
        --codesign-identity) XCODEGEN_MACARCHY_CODE_SIGN_IDENTITY="$2"; shift 2 ;;
...
entries() {
    for file in docs/macarchy-*.adoc; do
        ...
        subcommand=$(basename $file | sed 's/^macarchy-//' | sed 's/\.adoc$//')
...
if test $generate_xcodeproj = 1; then
    export XCODEGEN_MACARCHY_VERSION=$build_version
