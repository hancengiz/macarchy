...
/// becomes nil, etc.) which tricks Macarchy into thinking that all windows were closed.
/// That's why every time a window dies Macarchy caches the "entire world" (unless window is already presented in the cache)
/// so that once the screen is unlocked, Macarchy could restore windows to where they were
