...
:url-search-for-config: https://github.com/search?q=path%3A*macarchy.toml&type=code
...
Macarchy provides bash, fish and zsh completions.
...
. Unpack zip
. Put unpacked `macarchy-v$VERSION/macarchy.app` to `/Applications`
. Put unpacked `macarchy-v$VERSION/bin/macarchy` anywhere to `$PATH`
(The step is optional. It is only needed if you want to be able to interact with Macarchy from CLI)

If you see this message

----
"macarchy.app" can't be opened because Apple cannot check it for malicious software.
----

**Option 1** to resolve the problem

```
xattr -d com.apple.quarantine /Applications/macarchy.app
```

**Option 2** to resolve the problem

. navigate in Finder to `/Applications/macarchy.app`
. Right mouse click
. Open (yes, it's that stupid)

[#configuring-macarchy]
== Configuring Macarchy

[#config-location]
=== Custom config location

Macarchy tries to find the custom config in two locations:

. `~/.macarchy.toml`
. `+${XDG_CONFIG_HOME}/macarchy/macarchy.toml+`
...
Macarchy uses TOML 1.1.0 format for the config.
...
If no custom config is found, Macarchy will load the default config.
...
[source,shell]
----
cp /Applications/macarchy.app/Contents/Resources/default-config.toml ~/.macarchy.toml
----
...
The initial binding mode that Macarchy starts out with is "main".
...
Commands are the thing you use to manipulate Macarchy and query its state.

There are two ways on how you can use commands:

. Bind keys to run Macarchy commands. Example:
+
[source,toml]
----
[mode.main.binding]
...
Macarchy reimplements a small subset of Shell syntax.
...
Macarchy's shell syntax diverges from that dubious design decision.
...
*(2)* In Macarchy, the pipe operator `|` semantic matches `set -o pipefail` from traditional shells.
====
...
* For `dvorak` and `colemak` users, Macarchy offers preconfigured presets.
+
[source,toml]
----
...
`config-version` is a mechanism for introducing breaking changes to Macarchy.
...
You use `after-startup-command` to run arbitrary commands on Macarchy startup.

Common use cases:

. Run third-party tool on Macarchy startup (e.g. Sketchybar)
...
* `exec.inherit-env-vars = true` configures whether inherit environment variables of `macarchy.app` or not. (The default is `true`)
...
That's why unless you override `exec` section in your config, Macarchy falls back to the following `exec` configuration:
+
[source,toml]
----
[exec]
...
Macarchy stores all windows and containers in a tree.
Macarchy tree tiling model is https://i3wm.org/docs/userguide.html#tree[inspired by i3].

*Definition.* Each non-leaf node is called a "Container"

WARNING: i3 has a different terminology.
"container" in i3 is the same as "node" in Macarchy.
...
In total, Macarchy provides 4 possible layouts:
...
By default, Macarchy does two types of tree normalizations:
...
Since Spaces are so hard to deal with, Macarchy reimplements Spaces and calls them "Workspaces".
...
When you quit the Macarchy or when the Macarchy detects that it's about to crash, Macarchy will place all windows back to the visible area of the screen.

Macarchy shows the name of currently active workspace in its tray icon (top right corner), to give users a visual feedback on what workspace is currently active.

The intended workflow of using Macarchy workspaces is to only have one macOS Space (or as many monitors you have, if `Displays have separate Spaces` is enabled) and don’t interact with macOS Spaces anymore.
...
That means, that if Macarchy crashes badly you will still be able to manually "unhide" the windows by dragging these few pixels to the center of the screen.
...
Since Macarchy needs a free space to hide windows in,
...
For some reason, mission control doesn't like that Macarchy puts a lot of windows in the bottom right corner of the screen.
...
moving windows between monitors causes windows to move between different Spaces which is not correctly handled by the public APIs Macarchy uses,
...
|👍 Public Apple API are more stable (which in turn affects Macarchy stability)
...
. Macarchy processes the list of the callback objects in the defined order.
. For every callback object, Macarchy runs the command in the `if`.
. If the `if` command exits with zero exit code (successful), then Macarchy runs the command in the `run` and doesn't process any further callback objects.
. If the `if` command exits with non-zero exit code (fail), then Macarchy attempts processing the next object in the list.
...
* You can use xref:./commands.adoc#list-apps[`macarchy list-apps`] CLI command to get IDs of running applications
...
exec-on-workspace-change = ['/bin/bash', '-c',
    'sketchybar --trigger macarchy_workspace_change FOCUSED=$AEROSPACE_FOCUSED_WORKSPACE'
]
----
...
xref:./goodies.adoc#show-macarchy-workspaces-in-sketchybar[]

[#environment-variables]
== Environment variables

When Macarchy runs a callback's command (such as the `test`/`run` clause of an <<on-window-detected-callback>>, or the command in an <<on-focus-changed-callbacks>>),
...
on-window-detected = [
    ...
    # 'macarchy move-node-to-workspace W' moves the *newly detected* window,
    # because the child shell inherits AEROSPACE_WINDOW_ID pointing at it.
    { if = 'true', run = 'exec-and-forget sleep 1; macarchy move-node-to-workspace W' },
]
----
...
. Macarchy takes the assigned monitor of the workspace and makes the workspace visible on the monitor
. Macarchy focuses the workspace
...
[NOTE]
====
The only difference between Macarchy and i3 is switching to empty workspaces.
When you switch to an empty workspace, Macarchy puts the workspace on an assigned monitor; i3 puts the workspace on currently active monitor.

* I find that Macarchy model works better with <<observation,_the observation_>> listed above.
* Macarchy model is more consistent (it works the same for empty workspaces and non-empty workspaces)
====
...
Macarchy uses the API to gently ask windows whether they are dialogs, but Macarchy also applies some heuristics.
...
It's fine to hardcode special handling for popular applications, Macarchy already does it.
Please see `isDialogHeuristic` function in Macarchy sources.
...
If you can't make Macarchy handle some keys in your config, please make sure that you don't have keys conflict with other software that might listen to global keys (e.g. skhd, Karabiner-Elements, Raycast)

[#socket-protocol]
== Socket protocol

This section documents the client-server socket protocol that the `macarchy` CLI uses to talk to the running `macarchy.app` server.
It is intended for people who want to integrate with Macarchy from a custom client (e.g. a status bar, an editor plugin, a script in a language without a `macarchy` wrapper).

[#socket-location]
=== Socket location

Macarchy listens on a Unix-domain stream socket:

[source]
----
/tmp/com.hancengiz.macarchy-${USER}.sock        # macarchy.app (release builds)
/tmp/com.hancengiz.macarchy.debug-${USER}.sock  # macarchy-Debug.app (debug builds only)
----
...
Macarchy only runs on Apple Silicon and Intel macOS, both little-endian, so in practice the framing is always 4 bytes, little-endian.
...
| `args`
| array of strings
| The CLI arguments as you would pass them to `macarchy`, *excluding* the program name itself.
For example, `["workspace", "1"]` is the wire form of `macarchy workspace 1`.
...
Macarchy sets this variable when invoking callbacks so that the called commands know the window they was triggered from (see <<environment-variables>>).
...
| `0` on success, non-zero on failure. Mirrors what the `macarchy` CLI would have exited with.
...
When the client sends a request with `args` starting with `"subscribe"`, the server enters event-streaming mode. The remaining `args` follow the same syntax as the xref:./commands.adoc#subscribe[`macarchy subscribe`] CLI command (`--all`, `--no-send-initial`, or an explicit list of event types).
...
A minimal Python client that runs `macarchy list-workspaces --focused`:
...
sock_path = f"/tmp/com.hancengiz.macarchy-{getpass.getuser()}.sock"
