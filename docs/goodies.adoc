...
Do you have a cool automatization, Macarchy integration, or workflow?
...
You can also use `after-startup-command` to start JankyBorders together with Macarchy

[source,toml]
----
after-startup-command = [
    # JankyBorders has a built-in detection of already running process,
    # so it won't be run twice on Macarchy restart
    'exec-and-forget borders active_color=0xffe1e3e4 inactive_color=0xff494d64 width=5.0'
]
----

[#raycast-extension]
== Macarchy Raycast extension

There is a third party Raycast extension for Macarchy. https://www.raycast.com/limonkufu/aerospace
...
[source,bash]
----
macarchy eval 'list-workspaces --monitor mouse --visible | workspace --stdin next; workspace next --wrap-around'

macarchy eval 'list-workspaces --monitor mouse --visible | workspace --stdin next; workspace prev --wrap-around'
----
...
Beware that you might need to specify full path to macarchy executable https://community.folivora.ai/t/how-to-execute-terminal-command-to-switch-workspaces-with-aerospace/35914
...
[#show-macarchy-workspaces-in-sketchybar]
== Show Macarchy workspaces in Sketchybar

You can integrate Macarchy workspace indicators with link:https://github.com/FelixKratz/SketchyBar[Sketchybar].
Use these snippets as a starting point.

.~/.macarchy.toml
[source,toml]
----
# Run Sketchybar together with Macarchy
...
exec-on-workspace-change = ['/bin/bash', '-c',
    'sketchybar --trigger macarchy_workspace_change FOCUSED_WORKSPACE=$AEROSPACE_FOCUSED_WORKSPACE'
]
----
...
[source,bash]
----
sketchybar --add event macarchy_workspace_change

for sid in $(macarchy list-workspaces --all); do
    sketchybar --add item space.$sid left \
        --subscribe space.$sid macarchy_workspace_change \
        --set space.$sid \
        ...
        label="$sid" \
        click_script="macarchy workspace $sid" \
        script="$CONFIG_DIR/plugins/macarchy.sh $sid"
done
----

.~/.config/sketchybar/plugins/macarchy.sh
[source,bash]
----
#!/usr/bin/env bash

# make sure it's executable with:
# chmod +x ~/.config/sketchybar/plugins/macarchy.sh
...
[#show-macarchy-workspaces-in-simple-bar]
== Show Macarchy workspaces in simple-bar

link:https://github.com/Jean-Tinland/simple-bar[simple-bar] can be used to display Macarchy workspaces.

In order to sync simple-bar with Macarchy, add these lines to your Macarchy config file:
...
Opening *a new window* of a program that can support multiple windows (such as Safari or Terminal.app) can be accomplished with an AppleScript inlined in `macarchy.toml` as follows:
